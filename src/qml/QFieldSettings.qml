...
Page {
  ...
  Component.onCompleted: {
    if (appIsSungsan) {
      // Sungsan builds do not ship a Sentry DSN and must not inherit an old
      // opt-in value from settings restored by the operating system.
      registry.enableInfoCollection = false;
    }
  ...
  Settings {
    ...
    // Sungsan field workflow defaults geometry auto-save to on. Upstream
    // QField keeps its original opt-in default.
    property bool autoSave: appIsSungsan
    property bool fingerTapDigitizing: false
    property bool mouseAsTouchScreen: false
    property bool enableInfoCollection: !appIsSungsan
    ...
    Component.onCompleted: {
      for (var i = 0; i < count; i++) {
        if (get(i).settingAlias === 'nativeCamera2') {
          ...
          setProperty(i, 'isVisible', !appIsSungsan && (platformUtilities.capabilities & PlatformUtilities.SentryFramework) ? true : false);
        } else {
          setProperty(i, 'isVisible', true);
        }
      }
