...
Drawer {
  ...
  width: appIsSungsan
           ? Math.min(380, mainWindow.width)
           : Math.min(Math.max(330, closeButton.width + buttonsRow.width + menuButton.width + mainWindow.sceneLeftMargin + 1), mainWindow.width)
  height: parent.height
  edge: Qt.LeftEdge
  dragMargin: 10
  interactive: allowInteractive && (appIsSungsan || buttonsRowContainer.width >= buttonsRow.width)
  ...
  ColumnLayout {
    ...
    RowLayout {
      ...
      QfToolButton {
        ...
        height: 36
        padding: 0
        visible: !appIsSungsan && flatLayerTree.isTemporal
      ...
      QfToolButton {
        ...
        Layout.alignment: Qt.AlignVCenter
        visible: !appIsSungsan && (projectDescription != "" || projectAuthor != "")
        width: 36
        height: 36
        padding: 0
  ...
  Rectangle {
    ...
    Item {
      ...
      Label {
        ...
        text: modeSwitch.checked ? "편집 모드" : "보기 모드"
        font: Theme.strongFont
        color: modeSwitch.checked ? Theme.mainColor : Theme.mainTextColor
        elide: Text.ElideRight
      }
