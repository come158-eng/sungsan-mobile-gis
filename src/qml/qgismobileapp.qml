...
ApplicationWindow {
  ...
  Shortcut {
    ...
    sequence: "Ctrl+O"
    onActivated: {
      welcomeScreen.browseProjectsRequested();
    }
  }
  ...
  Item {
    ...
    MapCanvas {
      ...
      bottomMargin: allowMargins ? Math.max(informationDrawer.height > mainWindow.sceneBottomMargin ? informationDrawer.height : 0, !featureListForm.fullScreenView && !featureListForm.canvasOperationRequested && featureListForm.y > 0 ? featureListForm.height : 0, sungsanFieldPanel.visible ? sungsanFieldPanel.reservedBottom : 0) : 0
  ...
  Item {
    ...
    ScaleBar {
      ...
      anchors.bottomMargin: (sungsanFieldPanel.visible ? sungsanFieldPanel.reservedBottom : 0) + 10
    }
  ...
  function sungsanSaveCurrentWork(showConfirmation) {
    if (digitizingToolbar.isDigitizing) {
      displayToast("진행 중인 도형을 먼저 완료하거나 취소해 주세요.", "warning");
      return false;
    }

    let success = true;
    if (overlayFeatureFormDrawer.opened) {
      success = overlayFeatureFormDrawer.featureForm.save();
    } else if (featureListForm.state === "FeatureFormEdit") {
      success = featureListForm.saveCurrentForm();
    }

    if (!success) {
      displayToast("필수 입력값을 확인해 주세요. 저장하지 못했습니다.", "error");
      return false;
    }

    // Completed feature forms are committed by FeatureModel.  Persist the
    // current layer-tree state as part of an explicit manual save as well.
    projectInfo.saveLayerTreeState();
    if (!iface.saveProject()) {
      displayToast("프로젝트 파일을 저장하지 못했습니다.", "error");
      return false;
    }
    sungsanLastSavedText = new Date().toLocaleTimeString(Qt.locale(), "hh:mm");
    if (showConfirmation) {
      displayToast("현재까지 입력한 내용이 저장되었습니다. (" + sungsanLastSavedText + ")");
    }
    return true;
  }

  function sungsanExportCurrentProject() {
    if (!qgisProject || !qgisProject.fileName) {
      displayToast("내보낼 현장 프로젝트가 없습니다.", "warning");
      return;
    }
    if (!(platformUtilities.capabilities & PlatformUtilities.CustomSend)) {
      displayToast("이 기기에서는 ZIP 공유 기능을 사용할 수 없습니다.", "warning");
      return;
    }
    if (!sungsanSaveCurrentWork(false)) {
      return;
    }
    const projectDirectory = FileUtils.absolutePath(projectInfo.filePath);
    if (!gpkgFlusher.flushDirectory(projectDirectory)) {
  ...
  function sungsanRefreshVWorldStatus() {
    ...
    for (const layerId in layers) {
      ...
      if (layer && (layer.name === "VWorld 위성영상 · 출처: 국토교통부 브이월드" || ProjectUtils.isVWorldSatelliteLayer(layer))) {
        sungsanVWorldReady = true;
        return;
      }
    }
  }
  ...
  Timer {
    interval: 3000
    repeat: true
    running: qgisProject && !!qgisProject.fileName
    onTriggered: mainWindow.sungsanRefreshVWorldStatus()
  }

  Timer {
    // Feature edits are committed by the engine immediately; this five-minute
    // checkpoint persists project-level state without excessive mobile I/O.
    interval: 300000
    repeat: true
    running: qfieldSettings.autoSave && qgisProject && !!qgisProject.fileName
    onTriggered: {
      // Never interrupt an open form or a geometry that has not been confirmed.
      if (overlayFeatureFormDrawer.opened || featureListForm.visible || digitizingToolbar.isDigitizing || busyOverlay.state === "visible") {
        return;
      }
      projectInfo.saveLayerTreeState();
      if (iface.saveProject()) {
        mainWindow.sungsanLastSavedText = new Date().toLocaleTimeString(Qt.locale(), "hh:mm");
      } else {
        displayToast("자동 저장에 실패했습니다. 수동 저장을 눌러 확인해 주세요.", "error");
      }
    }
  }
  ...
  Connections {
    ...
    function onLoadProjectEnded(path, name) {
      ...
      if (path === '') {
        ...
        displayToast("프로젝트 파일을 읽을 수 없습니다. PC에서 패키지를 다시 만들어 주세요.", "error");
        return;
      }
  ...
  SungsanFieldPanel {
    ...
    onRemoveVertexRequested: {
      if (digitizingToolbar.isDigitizing) {
        digitizingToolbar.removeVertex();
      }
    }

    onConfirmGeometryRequested: {
      if (digitizingToolbar.geometryValid) {
        digitizingToolbar.confirm();
      }
    }

    onCancelGeometryRequested: {
      if (digitizingToolbar.isDigitizing) {
        digitizingToolbar.cancelDialog.open();
      }
    }

    onManualSaveRequested: mainWindow.sungsanSaveCurrentWork(true)

    onAutoSaveToggled: enabled => {
      qfieldSettings.autoSave = enabled;
      displayToast(enabled ? "도형 자동저장을 켰습니다." : "도형 자동저장을 껐습니다.");
    }
  ...
  SungsanHomeScreen {
    ...
    anchors.fill: parent
    hasCurrentProject: qgisProject && !!qgisProject.fileName
    currentProjectName: qgisProject && qgisProject.fileName ? ProjectUtils.title(qgisProject) : ""
    projectImportAvailable: platformUtilities.capabilities & PlatformUtilities.CustomImport

    onImportProjectRequested: {
      platformUtilities.requestStoragePermission();
      platformUtilities.importProjectArchive();
    }

    onBrowseProjectsRequested: {
      platformUtilities.requestStoragePermission();
    ...
    onOpenRecentProjectRequested: (path, title, projectType) => {
      if (projectType === RecentProjectListModel.LinkProject) {
        iface.importUrl(path, title, true);
      } else {
        iface.loadFile(path, title);
      }
    }

    onContinueSurveyRequested: mainWindow.sungsanStartSurvey()
    ...
    onOpenSourceInformationRequested: {
      aboutDialog.visible = true;
    }

    onCloseApplicationRequested: {
