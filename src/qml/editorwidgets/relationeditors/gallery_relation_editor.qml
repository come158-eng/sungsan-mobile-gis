  ...
  function resolveAttachmentPath(path) {
    ...
    if (externalStorage.type !== "") {
      ...
      if (authConfigId !== "" && !AuthUtils.isAuthenticationConfigurationAvailable(authConfigId)) {
        ...
        const message = qsTr("The external storage's authentication configuration ID is missing, please insure it is imported into %1").arg(appName);
        if (appIsSungsan) {
          mainWindow.displayToast(message, "error");
        } else {
          mainWindow.displayToast(message, "error", qsTr("Learn more"), function () {
