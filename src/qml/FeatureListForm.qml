...
Pane {
  ...
  // Public entry point used by Sungsan's persistent manual-save control.
  // FeatureForm.save() performs the same constraint checks and commit path as
  // the form's own save button.
  function saveCurrentForm() {
    if (state !== "FeatureFormEdit") {
      return true;
    }
    return featureForm.save();
  }
