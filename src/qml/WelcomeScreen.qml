...
Page {
              ...
              QfButton {
                ...
                onClicked: {
                  if (Qt.platform.os === "windows") {
                    ...
                    Qt.openUrlExternally("market://details?id=" + appPackageId);
      ...
      Rectangle {
        ...
        ColumnLayout {
          ...
          Container {
            ...
            readonly property real itemWidth: appIsSungsan ? welcomeActions.width / 2 : (count > 3 ? welcomeActions.width / 3.3 : welcomeActions.width / 3)
            ...
            QfWelcomeAction {
              ...
              width: visible ? welcomeActionsContainer.itemWidth : 0
  ...
  Rectangle {
    anchors.fill: parent
    color: "#00000000"
    visible: !appIsSungsan && imageDialLogo.value < 0.1
  ...
  Component.onCompleted: {
    if (appIsSungsan) {
      // The Sungsan shell has its own task-oriented help. Do not launch the
      // upstream QField map/dashboard tours or service prompts.
    ...
    if (!feedbackFormShown) {
      ...
      if (dt != "") {
        ...
        if (runCount >= runsToPrompt && (now - dt) >= (daysToPrompt * 24 * 60 * 60 * 1000)) {
          if (!appIsSungsan && appName === "QField") {
    ...
    if (!appIsSungsan && (platformUtilities.capabilities & PlatformUtilities.SentryFramework)) {
