  ...
  , mApp( app )
{
  ...
  QDesktopServices::setUrlHandler( qfield::urlScheme, mUrlHandler.get(), "handleUrl" );
...
void QgisMobileapp::initDeclarative( QQmlEngine *engine )
{
  ...
  engine->rootContext()->setContextProperty( "appPackageId", qfield::appPackageId );
  engine->rootContext()->setContextProperty( "appIsSungsan", qfield::isSungsanBuild );
...
void QgisMobileapp::readProjectFile()
{
  ...
  if ( !fi.exists() )
  {
    ...
    QSettings().remove( QStringLiteral( "QField/lastProjectFilePath" ) );
  ...
  bool projectLoadAttempted = false;
  if ( SUPPORTED_PROJECT_EXTENSIONS.contains( suffix ) )
  {
    projectLoadAttempted = true;
    projectLoaded = mProject->read( mProjectFilePath, Qgis::ProjectReadFlag::DontLoadProjectStyles | Qgis::ProjectReadFlag::DontLoad3DViews );
  }
  else if ( suffix == QStringLiteral( "gpkg" ) )
  {
    ...
    if ( storage )
    {
      ...
      if ( !projectNames.isEmpty() )
      {
        ...
        projectLoaded = mProject->read( QgsGeoPackageProjectStorage::encodeUri( projectUri ), Qgis::ProjectReadFlag::DontLoadProjectStyles | Qgis::ProjectReadFlag::DontLoad3DViews );
      }
    }
  }

  // Never continue with a partially initialized QgsProject. Apart from
  // presenting a corrupt archive as successfully opened, doing so would let a
  // later save rewrite the unreadable desktop project with incomplete state.
  if ( projectLoadAttempted && !projectLoaded )
  {
    const QString failedPath = mProjectFilePath;
    const QString projectError = mProject->error();
    QgsMessageLog::logMessage(
      projectError.isEmpty()
        ? tr( "Could not read project file \"%1\"" ).arg( failedPath )
        : tr( "Could not read project file \"%1\": %2" ).arg( failedPath, projectError ),
      QStringLiteral( "QField" ),
      Qgis::Critical
    );
    mProject->clear();
    mProjectFilePath.clear();
...
bool QgisMobileapp::clearProject()
{
  const QString projectDirectory = mProjectFilePath.isEmpty() ? QString() : QFileInfo( mProjectFilePath ).absolutePath();
  mAuthRequestHandler->clearStoredRealms();

  mProject->clear();
  const bool databasesFlushed = projectDirectory.isEmpty() || !mGpkgFlusher || mGpkgFlusher->flushDirectory( projectDirectory );
  if ( !databasesFlushed )
  {
    QgsMessageLog::logMessage( tr( "Could not checkpoint every project database while closing the project" ), qfield::appName, Qgis::Critical );
  }
  mProjectFileName = QString();
  mProjectFilePath = QString();
  return databasesFlushed;
}
