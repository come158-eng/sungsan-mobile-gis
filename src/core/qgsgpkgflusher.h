...
class QgsGpkgFlusher : public QObject
{
  ...
  public:
    ...
    /**
     * Synchronously checkpoints GeoPackage/SQLite WAL files below
     * a directoryPath. This is used immediately before a project directory is
     * archived so the base database and WAL cannot be copied across a pending
     * background checkpoint.
     *
     * Returns TRUE when every pending database was checkpointed.
     */
    Q_INVOKABLE bool flushDirectory( const QString &directoryPath );
