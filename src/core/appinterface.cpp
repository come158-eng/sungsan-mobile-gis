...
bool AppInterface::saveProject()
{
  ...
  // Feature forms are committed by FeatureModel before this checkpoint. Do
  // not rewrite an older desktop project with a newer major QGIS project
  // schema: the user's QGIS 3.40.14 must be able to reopen the same QGZ after
  // field data has been edited. Sungsan stores its mobile layer-tree state in
  // ProjectInfo/QSettings, so skipping the QGZ rewrite does not lose that UI
  // state. Projects created by the running major version still use the normal
  // QgsProject::write() path below.
  if ( qfield::isSungsanBuild )
  {
    const QgsProjectVersion sourceVersion = project->lastSaveVersion();
    if ( sourceVersion.isNull() )
    {
      QgsMessageLog::logMessage(
        tr( "The project source version is unknown; its QGS/QGZ was not rewritten" ),
        qfield::appName,
        Qgis::Warning
      );
      return false;
    ...
    const int runningMajorVersion = Qgis::version().section( '.', 0, 0 ).toInt();
    if ( !sourceVersion.isNull() && runningMajorVersion > 0 && sourceVersion.majorVersion() < runningMajorVersion )
    {
      QgsMessageLog::logMessage(
        tr( "Field data was saved without upgrading the desktop project file format" ),
        qfield::appName,
        Qgis::Info
      );
      return true;
  ...
  const bool written = project->write();
  if ( !written )
  {
    QgsMessageLog::logMessage(
      tr( "The current project could not be saved" ),
      qfield::appName,
      Qgis::Warning
    );
  }
...
bool AppInterface::clearProject() const
{
  QgisMobileapp *mobileApp = app();
  if ( mobileApp )
  {
    return mobileApp->clearProject();
  }
  return false;
}
