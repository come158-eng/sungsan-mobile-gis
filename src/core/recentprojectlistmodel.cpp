...
void RecentProjectListModel::reloadModel()
{
  ...
  {
    // The Sungsan workflow starts from a PC-created field package. Upstream
    // sample links would make the independent app look like QField again.
    settings.setValue( QStringLiteral( "QField/recentProjectsAdded" ), true );
  }
  else if ( !sampleProjectsAdded )
  {
...
QList<RecentProjectListModel::RecentProject> RecentProjectListModel::recentProjects( bool skipNonAvailable )
{
  ...
  for ( int i = 0; i < projectKeys.count(); i++ )
  {
    ...
    {
      if ( qfield::isSungsanBuild || ( skipNonAvailable && ( !fi.exists() || !path.startsWith( QStringLiteral( "%1%2%3%2" ).arg( qdieldCloudLocalDirectory, QDir::separator(), qfieldCloudUsername ) ) ) ) )
      {
        skip = true;
      }
