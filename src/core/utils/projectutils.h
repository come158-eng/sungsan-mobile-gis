...
class ProjectUtils : public QObject
{
  ...
  public:
    ...
    /**
     * Returns whether a map \a layer uses the official VWorld satellite WMTS endpoint.
     * The provider source itself is deliberately not exposed to QML because it may
     * contain credentials.
     */
    Q_INVOKABLE static bool isVWorldSatelliteLayer( QgsMapLayer *layer );
    ...
    /**
     * Adds a map \a layer to the bottom of a \a project's layer tree.
     * This is intended for background maps which must not cover operational layers.
     */
