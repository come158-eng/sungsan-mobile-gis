...
QVariantMap UrlUtils::getActionDetails( const QString &url )
{
  ...
  QUrl actionUrl( url );
  if ( actionUrl.scheme().compare( qfield::urlScheme, Qt::CaseInsensitive ) == 0 )
  {
...
{
  QUrl url;
  if ( scheme.compare( QStringLiteral( "qfield" ), Qt::CaseInsensitive ) == 0
       || scheme.compare( qfield::urlScheme, Qt::CaseInsensitive ) == 0 )
  {
    url.setScheme( qfield::urlScheme );
