...
#define JNI_FUNCTION_NAME( package_name, class_name, function_name ) GLUE_HELPER( Java_, package_name, _, class_name, _, function_name )
...
{
  AppInterface *iface = AppInterface::instance();
  if ( iface )
  {
    const char *pathStr = env->GetStringUTFChars( path, NULL );
    const QString projectPath = QString::fromUtf8( pathStr );
    env->ReleaseStringUTFChars( path, pathStr );

    const auto loadProject = [iface, projectPath]() { iface->loadFile( projectPath ); };
    if ( QThread::currentThread() == iface->thread() )
    {
      loadProject();
    }
    else if ( !QMetaObject::invokeMethod( iface, loadProject, Qt::BlockingQueuedConnection ) )
    {
      qCritical() << "Could not marshal Android project opening to the Qt thread";
    }
  }
  return;
}

JNIEXPORT jboolean JNICALL JNI_FUNCTION_NAME( APP_PACKAGE_JNI_NAME, QFieldActivity, clearProject )( JNIEnv *env, jobject obj )
{
  AppInterface *iface = AppInterface::instance();
  if ( !iface )
  {
  ...
  bool projectClosedSafely = false;
  const auto clearCurrentProject = [iface, &projectClosedSafely]() { projectClosedSafely = iface->clearProject(); };
  if ( QThread::currentThread() == iface->thread() )
  {
    clearCurrentProject();
    return projectClosedSafely ? JNI_TRUE : JNI_FALSE;
  }

  if ( !QMetaObject::invokeMethod( iface, clearCurrentProject, Qt::BlockingQueuedConnection ) )
  {
    qCritical() << "Could not marshal Android project closing to the Qt thread";
    return JNI_FALSE;
  }
  return projectClosedSafely ? JNI_TRUE : JNI_FALSE;
}
