...
void PlatformUtilities::initSystem()
{
  ...
  gitRevFile.close();
  // A branded source archive can be built without .git, and multiple branded
  // releases can share the same upstream commit.  Include the release string
  // so bundled plugins, themes and other assets are recopied on app updates.
  QByteArray appGitRev = ( qfield::gitRev + QStringLiteral( "|" ) + qfield::appVersionStr ).toUtf8();
...
void PlatformUtilities::importLandStarPoints() const
{}
