...
if [[ -z "${KEYNAME:-}" ]]; then
  echo "오류: 메타이엔지 배포 서명 키 별칭이 지정되지 않았습니다." >&2
  exit 2
fi
...
validation_directory="$(mktemp -d "${TMPDIR:-/tmp}/metaengi-keystore-check.XXXXXX")"
...
echo "메타이엔지 배포 서명 키스토어 사전 검증을 통과했습니다."
