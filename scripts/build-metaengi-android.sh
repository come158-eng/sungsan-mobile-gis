...
OUTPUT_DIR="${OUTPUT_DIR:-${SOURCE_DIR}/build-metaengi-apk}"
export triplet="${triplet:-arm64-android}"
SOURCE_BUILD_DIR="${SOURCE_DIR}/build-metaengi-native-${triplet}"
GENERATED_BUILD_DIR="${SOURCE_DIR}/build-metaengi-native-generated"
...
for existing_path in \
  ...
  if [[ -e "${existing_path}" || -L "${existing_path}" ]]; then
    echo "오류: 이전 메타이엔지 빌드 경로가 남아 있습니다: ${existing_path}" >&2
    echo "새 소스 사본에서 빌드하거나, 위 전용 경로를 직접 보관/정리한 뒤 다시 실행하세요." >&2
    exit 5
  fi
done

if [[ -z "${METAENGI_VWORLD_API_KEY:-}" ]]; then
  echo "오류: METAENGI_VWORLD_API_KEY 환경 변수에 VWorld API 키를 지정하세요." >&2
  exit 2
fi
...
ALLOW_UNSIGNED_TEST_BUILD="${METAENGI_ALLOW_UNSIGNED_TEST_BUILD:-0}"
if [[ "${ALLOW_UNSIGNED_TEST_BUILD}" != "0" && "${ALLOW_UNSIGNED_TEST_BUILD}" != "1" ]]; then
  echo "오류: METAENGI_ALLOW_UNSIGNED_TEST_BUILD는 0 또는 1만 사용할 수 있습니다." >&2
  exit 9
fi
UNSIGNED_RELEASE_ONLY="${METAENGI_BUILD_UNSIGNED_RELEASE_ONLY:-0}"
if [[ "${UNSIGNED_RELEASE_ONLY}" != "0" && "${UNSIGNED_RELEASE_ONLY}" != "1" ]]; then
  echo "오류: METAENGI_BUILD_UNSIGNED_RELEASE_ONLY는 0 또는 1만 사용할 수 있습니다." >&2
  exit 18
fi
...
if [[ "${UNSIGNED_RELEASE_ONLY}" == "1" ]]; then
  ...
  echo "내부 시험용 디버그키 빌드만 METAENGI_ALLOW_UNSIGNED_TEST_BUILD=1로 명시할 수 있습니다." >&2
  exit 8
fi
...
if [[ "${BUILD_KIND}" == "signed_release" ]]; then
  ...
  if [[ -z "${METAENGI_KEYSTORE_PATH:-}" ]]; then
    echo "오류: METAENGI_KEYSTORE_PATH에 소스 폴더 밖의 배포 키스토어를 지정하세요." >&2
    exit 7
  fi
  if [[ "${METAENGI_KEYSTORE_PATH}" == *:* ||
        "${METAENGI_KEYSTORE_PATH}" == *$'\n'* ||
        "${METAENGI_KEYSTORE_PATH}" == *$'\r'* ]]; then
    echo "오류: Docker 안전 마운트를 위해 키스토어 경로에 콜론·줄바꿈을 사용할 수 없습니다." >&2
    exit 10
  fi
  if [[ -L "${METAENGI_KEYSTORE_PATH}" || ! -f "${METAENGI_KEYSTORE_PATH}" ||
        ! -r "${METAENGI_KEYSTORE_PATH}" ]]; then
    echo "오류: 배포 키스토어는 현재 사용자가 읽을 수 있는 일반 파일이어야 합니다." >&2
    exit 10
  fi
  KEYSTORE_HOST_PATH="$(realpath -e -- "${METAENGI_KEYSTORE_PATH}")"
...
# androiddeployqt must never see release-signing state for Meta Engineering. Qt 6.10.x
...
export QFIELD_CMAKE_BUILD_DIR="/usr/src/qfield/build-metaengi-native-${triplet}"
export CMAKE_BUILD_TYPE="Release"
export APP_NAME="metaeng mobile gis"
export APP_PACKAGE_NAME="metaengimobilegis"
export APP_PACKAGE_ID="kr.co.metaengi.mobilegis"
export APP_UPSTREAM_REVISION="f7123fc8dfa40be4e874d9bf5b46e81c6d05039b"
export APP_ICON="metaengi_mobile_gis"
export APP_ICON_PATH="/usr/src/qfield/branding/metaengi/assets"
# Use the transparent vector mark; the old raster splash had an opaque white box.
export APP_SPLASH_PATH=""
export APP_THEME_PATH="/usr/src/qfield/branding/metaengi/theme.json"
export APP_DEFAULT_LANGUAGE="ko"
export APP_URL_SCHEME="metaengimobilegis"
export APP_DATA_DIR_NAME="MetaEngiMobileGIS"
export APP_POSITIONING_CHANNEL_NAME="metaeng mobile gis"
export APP_POSITIONING_CHANNEL_DESCRIPTION="메타이엔지 위치 서비스"
export APP_POSITIONING_NOTIFICATION_TITLE="메타이엔지 위치 서비스"
export APP_POSITIONING_NOTIFICATION_RUNNING_TEXT="메타이엔지 위치 서비스가 실행 중입니다"
export APP_CLOUD_CHANNEL_NAME="metaeng mobile gis"
export APP_CLOUD_CHANNEL_DESCRIPTION="메타이엔지 데이터 전송 서비스"
export APP_CLOUD_NOTIFICATION_TITLE="메타이엔지 데이터 전송"
export APP_CLOUD_NOTIFICATION_RUNNING_TEXT="현장 첨부 파일을 전송하고 있습니다"
export APP_BUNDLED_PLUGINS="/usr/src/qfield/build-metaengi-native-generated/plugins"
...
export SUNG_SAN_VWORLD_API_KEY="${METAENGI_VWORLD_API_KEY}"

BUILD_MARKER="$(mktemp "${TMPDIR:-/tmp}/metaengi-build-marker.XXXXXX")"
SIGNING_STATE_DIR=""
HOST_USER_UID="$(id -u)"
HOST_USER_GID="$(id -g)"
cleanup() {
  ...
  if [[ -n "${SIGNING_STATE_DIR}" ]]; then
    rm -f -- \
      ...
      "${SIGNING_STATE_DIR}/bin/validate-metaengi-keystore.sh" \
      "${SIGNING_STATE_DIR}/bin/sign-metaengi-release-apk.sh"
    rmdir -- \
...
if [[ "${BUILD_KIND}" == "signed_release" ]]; then
  ...
  (
    unset STOREPASS KEYNAME KEYPASS METAENGI_KEYSTORE_PATH
    DOCKER_BUILDKIT=1 docker build "${SOURCE_DIR}/.docker/android_dev" -t qfield_and_dev
  )
  export SUNG_SAN_DOCKER_IMAGE_READY="1"
  SIGNING_STATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/metaengi-signing-state.XXXXXX")"
  mkdir -m 0700 \
    "${SIGNING_STATE_DIR}/anchor" \
    "${SIGNING_STATE_DIR}/secrets" \
    "${SIGNING_STATE_DIR}/bin"
  cp -- \
    "${SOURCE_DIR}/scripts/validate-metaengi-keystore.sh" \
    "${SIGNING_STATE_DIR}/bin/validate-metaengi-keystore.sh"
  cp -- \
    "${SOURCE_DIR}/scripts/sign-metaengi-release-apk.sh" \
    "${SIGNING_STATE_DIR}/bin/sign-metaengi-release-apk.sh"
  chmod 0500 \
    "${SIGNING_STATE_DIR}/bin/validate-metaengi-keystore.sh" \
    "${SIGNING_STATE_DIR}/bin/sign-metaengi-release-apk.sh"
  ...
  docker run --rm \
    ...
    -v "${KEYSTORE_HOST_PATH}:/run/secrets/metaengi-release-keystore.p12:ro" \
    ...
    -v "${SIGNING_STATE_DIR}/bin/validate-metaengi-keystore.sh:/run/bin/validate-metaengi-keystore.sh:ro" \
    -v "${SIGNING_STATE_DIR}/anchor:/run/anchor:rw" \
    -e KEYNAME \
    -e HOME=/tmp \
    -e SUNG_SAN_CERT_DIGEST_OUTPUT=/run/anchor/prebuild-certificate.sha256 \
    qfield_and_dev \
    /run/bin/validate-metaengi-keystore.sh
...
if [[ "${BUILD_KIND}" != "debug_test" ]]; then
  ...
  if [[ "${BUILD_KIND}" == "signed_release" ]]; then
    docker run --rm \
      ...
      -v "${SIGNING_STATE_DIR}/bin/validate-metaengi-keystore.sh:/run/bin/validate-metaengi-keystore.sh:ro" \
      -v "${SIGNING_STATE_DIR}/bin/sign-metaengi-release-apk.sh:/run/bin/sign-metaengi-release-apk.sh:ro" \
      -e KEYNAME \
      -e HOME=/tmp \
      qfield_and_dev \
      /run/bin/sign-metaengi-release-apk.sh
...
if [[ "${BUILD_KIND}" == "signed_release" ]]; then
  ...
  cp -f -- "${SIGNING_STATE_DIR}/anchor/prebuild-certificate.sha256" \
    "${OUTPUT_DIR}/MetaEngi-release-certificate.sha256"
  chmod 0644 \
    "${RELEASE_OUTPUT_APK}" \
    "${OUTPUT_DIR}/MetaEngi-release-certificate.sha256"
