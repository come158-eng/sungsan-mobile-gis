...
VALIDATOR="${SUNG_SAN_KEYSTORE_VALIDATOR:-/run/bin/validate-metaengi-keystore.sh}"
ZIPALIGN="${BUILD_TOOLS_DIR}/zipalign"
APKSIGNER="${BUILD_TOOLS_DIR}/apksigner"

if [[ -z "${KEYNAME:-}" ]]; then
  echo "오류: 메타이엔지 APK 서명 키 별칭이 지정되지 않았습니다." >&2
  exit 2
fi
...
if [[ -e "${SIGNED_APK}" || -L "${SIGNED_APK}" ]]; then
  echo "오류: 이전 메타이엔지 서명 APK가 남아 있어 덮어쓰지 않습니다." >&2
  exit 4
fi

unsigned_size="$(stat -c '%s' "${UNSIGNED_APK}")"
if [[ ! "${unsigned_size}" =~ ^[0-9]+$ ]] || (( unsigned_size < 1 )); then
  echo "오류: 메타이엔지 unsigned release APK가 비어 있습니다." >&2
  exit 5
fi
...
current_digest_file="$(mktemp "${TMPDIR:-/tmp}/metaengi-current-certificate.XXXXXX")"
aligned_apk="$(mktemp "${TMPDIR:-/tmp}/metaengi-aligned.XXXXXX.apk")"
verification_report="$(mktemp "${TMPDIR:-/tmp}/metaengi-apksigner-report.XXXXXX")"
...
echo "메타이엔지 release APK의 정렬·서명·인증서 검증을 통과했습니다."
