...
if(ANDROID AND ANDROIDDEPLOYQT_EXECUTABLE)
    ...
    if (NOT APP_PACKAGE_ID STREQUAL "ch.opengis.qfield")
        set(STAGED_SRC_FOLDER "${ANDROID_TEMPLATE_FOLDER}/src/.qfield-java-source")
        file(REMOVE_RECURSE "${STAGED_SRC_FOLDER}")
        file(RENAME "${DEFAULT_SRC_FOLDER}" "${STAGED_SRC_FOLDER}")
        file(REMOVE_RECURSE "${SRC_FOLDER}") # remove any pre-existing content
        get_filename_component(SRC_PARENT_FOLDER "${SRC_FOLDER}" DIRECTORY)
        file(MAKE_DIRECTORY "${SRC_PARENT_FOLDER}")
        file(RENAME "${STAGED_SRC_FOLDER}" "${SRC_FOLDER}")
    endif()
    file(GLOB JAVA_FILES "${SRC_FOLDER}/*.java")
    foreach(JAVA_FILE ${JAVA_FILES})
      message(STATUS ${JAVA_FILE})
      file(READ ${JAVA_FILE} CONTENT)
      string(REPLACE "ch.opengis.qfield" "${APP_PACKAGE_ID}" CONTENT "${CONTENT}")
      string(REPLACE "scheme.equals(\"qfield\")"
                     "scheme.equals(\"${APP_URL_SCHEME}\")"
                     CONTENT "${CONTENT}")
      string(REPLACE "QField/" "${APP_DATA_DIR_NAME}/" CONTENT "${CONTENT}")
      string(REPLACE "\"QField\"" "\"${APP_NAME}\"" CONTENT "${CONTENT}")
      string(REPLACE "R.drawable.qfield_logo"
                     "R.drawable.${APP_ICON}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_POSITIONING_CHANNEL_NAME@"
                     "${APP_POSITIONING_CHANNEL_NAME}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_POSITIONING_CHANNEL_DESCRIPTION@"
                     "${APP_POSITIONING_CHANNEL_DESCRIPTION}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_POSITIONING_NOTIFICATION_TITLE@"
                     "${APP_POSITIONING_NOTIFICATION_TITLE}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_POSITIONING_NOTIFICATION_RUNNING_TEXT@"
                     "${APP_POSITIONING_NOTIFICATION_RUNNING_TEXT}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_CLOUD_CHANNEL_NAME@"
                     "${APP_CLOUD_CHANNEL_NAME}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_CLOUD_CHANNEL_DESCRIPTION@"
                     "${APP_CLOUD_CHANNEL_DESCRIPTION}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_CLOUD_NOTIFICATION_TITLE@"
                     "${APP_CLOUD_NOTIFICATION_TITLE}"
                     CONTENT "${CONTENT}")
      string(REPLACE "@APP_CLOUD_NOTIFICATION_RUNNING_TEXT@"
