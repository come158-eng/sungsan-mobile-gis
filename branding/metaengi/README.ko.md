...
메타이엔지 전용 기본 문자열은 `branding/metaengi/android/res/values/strings.xml`에
분리되어 있습니다. 빌드할 때 패키지 ID가 정확히
`kr.co.metaengi.mobilegis`인 경우에만 임시 Android 패키징 폴더의 기본
`values/strings.xml`을 이 파일로 교체합니다. 따라서 일반 upstream 빌드의 영어
기본 문자열은 변경되지 않습니다. 메타이엔지 패키징 중에는 휴대폰의 다른 시스템 언어가
upstream 번역을 우선 선택하지 않도록 임시 패키징 폴더의 `values-*/strings.xml`만
제외하고, 모든 언어가 검토한 메타이엔지 한국어 기본 문구로 돌아가게 합니다. 원본 소스의
...
```bash
export METAENGI_VWORLD_API_KEY='발급받은-VWorld-키'
export STOREPASS='배포-키스토어-암호'
export KEYNAME='배포-키-별칭'
export KEYPASS='배포-키-암호'
export METAENGI_KEYSTORE_PATH='/소스/폴더/밖/keystore.p12'
# 키스토어 파일은 chmod 600 또는 400이어야 합니다.
./scripts/build-metaengi-android.sh
```

성산 배포에 사용한 동일한 `keystore.p12`, `STOREPASS`, `KEYNAME`, `KEYPASS`를
그대로 재사용해도 됩니다. 메타이엔지 앱은 별도 패키지 ID
`kr.co.metaengi.mobilegis`로 서명되므로, 같은 서명 인증서를 사용해도 성산 앱과
동시에 설치할 수 있습니다. 이 값들은 소스 파일이나 Git 저장소에 기록하지 말고
빌드 실행 환경 변수 또는 GitHub Actions Secret으로만 전달하세요.

기본 대상은 현재 Android 휴대폰 대부분에 맞는 `arm64-android`입니다. 다른 ABI가 필요하면 다음처럼 지정합니다.

```bash
triplet=arm-neon-android ./scripts/build-metaengi-android.sh
```

생성된 APK는 `build-metaengi-apk/`에 복사됩니다. 배포용 APK는 Android 서명 키가 필요하며 `STOREPASS`, `KEYNAME`, `KEYPASS`와 소스 폴더 밖의 `METAENGI_KEYSTORE_PATH`를 사용합니다. 소스 폴더 안에 키스토어를 넣으면 안전을 위해 빌드가 중단됩니다.

빌드 스크립트는 이전 CMake 캐시, 생성된 VWorld 플러그인 또는 복사된 APK가
남아 있으면 중단합니다. `build-metaengi-native-<ABI>/`,
`build-metaengi-native-generated/`, `build-metaengi-apk/`를 자동 삭제하지 않으므로,
...
`METAENGI_ALLOW_UNSIGNED_TEST_BUILD=1`을 명시할 수 있습니다. 이 경우 결과 파일명은
강제로 `DEBUG-KEY-TEST-ONLY-*.apk`가 되며 Android 디버그키 설치 시험 전용이므로
배포하면 안 됩니다. `METAENGI_BUILD_UNSIGNED_RELEASE_ONLY=1`은 보호된 CI 서명 작업에
넘길 서명 전 입력을 만들기 위한 모드이며, 결과 파일은 설치·배포하면 안 됩니다.

### GitHub Actions에서 APK 만들기

`.github/workflows/metaengi-android.yml`은 사람이 직접 시작하는
...
ZIP의 `.github/workflows/`에는 `metaengi-android.yml`만 포함하고, 이 비활성
참고 파일을 포함한 나머지 upstream 워크플로는 모두 제외합니다.

서명 배포본을 만들기 전에 GitHub 저장소에서 다음 설정을 반드시
완료해야 합니다.

1. `Settings → Environments`에서 `metaengi-release` 환경을 만듭니다.
2. 해당 환경에 **Required reviewers**를 지정하고 관리자 우회를
   허용하지 않도록 설정합니다.
3. `metaengi-release` 환경 암호로 아래 네 개를 추가합니다.
...
5. `Actions → Meta Engineering Android APK (manual and isolated) → Run workflow`에서
   `test_only` 또는 `signed_release`를 선택합니다.

GitHub의 환경 필수 승인자는 YAML 파일로 강제할 수 없습니다.
따라서 저장소 설정에 `metaengi-release` 필수 승인자가 없다면
...
메타이엔지 빌드는 공식 QField의 일반 빌드 캐시와 섞이지 않도록
`build-metaengi-native-<ABI>/` 전용 CMake 디렉터리를 사용합니다. Release 구성으로
빌드하며 Sentry DSN과 환경값은 빈 값으로 고정해 공식 QField 오류수집 주소가
메타이엔지 앱에 들어가지 않게 합니다.

배포용 대응 소스 ZIP에는 `.git` 폴더를 넣지 않습니다. 메타이엔지 빌드 스크립트는 이
경우에도 오픈소스 정보 화면이 깨진 주소를 만들지 않도록 감사한 QField v4.2.11
원본 기준 커밋 `f7123fc8dfa40be4e874d9bf5b46e81c6d05039b`를
`APP_UPSTREAM_REVISION`으로 고정합니다. 이 고정은 메타이엔지 패키지 ID에만 적용되며,
...
앱 시작 화면과 지도 위 현장 패널은 메타이엔지 전용 QML로 구성합니다. 일반 작업자는
...
이 포크는 원본 QField의 GNU GPL v2 이상 라이선스와 저작권 고지를 그대로 유지합니다. APK를 제3자에게 배포할 때에는 해당 APK에 대응하는 전체 소스 코드와 라이선스 고지를 함께 제공해야 합니다. 메타이엔지 로고의 사용 권리는 별도로 관리해야 합니다.
